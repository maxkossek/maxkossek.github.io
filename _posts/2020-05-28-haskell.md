...
To open an interactive prompt use `ghci`: `$ ghci`. Use the `:load` or `:l` command to load a Haskell source file into the interactive session. The `:reload` command reloads a source file. The `import` command loads a module into the interactive session. Use the `:set +s` command to measure the execution time of a command. Type `:quit` or `:q` to exit the interactive GHCi.
...
The "do" keyword links some number of actions together. All actions in a "do" block must be aligned with the first non-whitespace character after the "do" keyword.
```
getInput = do
  inputStr <- getLine
  putStrLn ("Input: " ++ InputStr)
```

The "let" keyword is used to create a local binding. Haskell is lexially scoped, so a "let" binding overrides references that are further outisde of the scope. In the example below, the argument `x` is never used.
```
printer x =
  let x = 10; y = 5 in
    x + y
...
A "let" binding inside an action does not require the "in" keyword.
```
rev = do
  inputStr <- getLine
...
The `:kind` or `:k` interactive command evaluates the kind of a data type. A data type of kind `* -> *` maps one kind `*` to another kind `*`. Higher-kinded types are not types until they are fully applied.
```
> :k Int
Int :: *
...
`Rational`
: Rational numbers that can be represented as the ratio of two integers.
...
Type names and constructor function names must start with a capital letter. A "data" declaration creates an user-defined type. A "type" declaration defines an alternative name for an existing types. A "newtype" declaration defines a new types as equivalent to an existing one. The `type` and `newtype` custom data types ensure no additional run-time overhead.
```
newtype Negatives = Negatives Integer deriving (Eq, Show)
...
The syntax of a `data` declaration is `data DataType = ConstructorName [types] | ConstructorName2 [types] | ...`. An enumeration is a `data` declaration where none of the constructors take in any arguments.
```
data Month = January | February | March

data Bool = T | F

data Time = ITime Int Int Int | StrTime String String String | Bool
  deriving (Eq, Show)
```
...
The "deriving" keyword automatically generates instance definitions for a data type. It can be used on the following built-in classes: `Eq`, `Ord`, `Enum`, `Bounded`, `Show`, and `Read`. Compared to "instance", using "deriving" is faster but also gives you less control over custom functionality.
```
data IntStr = IntStr {i :: Integer, s :: String}
  deriving (Eq, Ord, Show)
```

You can define your own type class using the "class" keyword. Inside the class, you can define a function's behavior like normal. It is possible to recursively define functions based on other functions in the class, such as in the standard `Prelude` `Eq` class.
```
class Eq a where
  (==), (/=) :: a -> a -> Bool
  x == y = not (x /= y)
...
`Fractional`
: Subclass of the `Num` type class including: `Float`, `Double`, `Rational`, and `Scientific`.
...
`Monoid`
: Class of objects that fulfill the identity and associativity properties: `x <> mempty = x`, `mempty <> x = x`, `(x <> y) <> z = x <> (y <> z)`.
...
The `fmap` function is similar to `map`, but for the `Functor` type class. When applied to a `Maybe` data type, it only performs the operation if the value is not `Nothing`. It takes in a function and a list that can be mapped over and produces the result of applying the function to each element in the list.
```
...
The `fold` family of functions take in a function and a list, and combine all of the elements in the list into a return value. The `foldr` function is right-associative--it folds a list from right to left. The first argument to `foldr` is a function that takes two arguments, and the second argument is the initial value for the accumulator. The `foldl` function is left-associative. The `foldl'` function is a version of `foldl` that forces the second function argument to be evaluated immediately, which makes it more efficient. Prefer using `foldl'` for most cases involving finite and large lists, and `foldr` when working with potentially infinite lists. The `foldl` function should almost never be used.
```
> :t foldl
...
Arithmetic operators: +, -, *, / (floating-point division), ^ (integer exponentiation), ^^ (fractional exponentiation), ** (floating-point exponentiation), `div` (integer division), `mod` (modulus), `quot` (integral division), `rem` (remainder). Negated numbers should be placed inside parentheses to avoid them being parsed as a subtraction.
```
> 3 * (5 + 2)
21
...
The `:info` or `:i` interactive command displays information about an operator, such as its associativity (l/r), precedence, and whether it is infix or prefix.
```
> :i (+)
> :i (+)
class Num a where
...
Operators acting on two operands that consist only of non-alphanumeric characters can be written as infix or as prefix. Functions that are alphanumeric are prefix by default, while functions with a symbol name are infix by default. An infix operator can be written as prefix by surrounding it with parenthesis "()" . A prefix operator can be written as infix by surrounding it with backticks "`".
```
> 4 + 9 == 13
True
...
Sectioning involves placing a single argument and operator inside a parenthesis. For commutative functions, the order does not matter.
```
> (+1) 2
3
> (1+) 2
...
The `String` type is a synonym for a list of `Char` values. Strings are enclosed in double quotes `""` to differentiate them from a `Char`, which is enclosed in single quotes `''`.
```
> :t "h"
"h" :: [Char]
> :t 'h'
...
The `++` concatenation operator or the `concat` function can be used to concatenate any number of strings.
```
> :t (++)
(++) :: [a] -> [a] -> [a]
> "hello" ++ " " ++ "world"
...
The dollar sign operator ("$") performs a simple function application, but with low precedence. The operator is commonly used to decrease the amount of parenthesis needed for an expression. For example, `f g $ h x` would be equivalent to `(f g) (h x)`; `f $ g h x` is equivalent to `f ((g h) x)`.
```
> :t ($)
($) :: (a -> b) -> a -> b
...
The `(.)` function composition operator is a short-hand for writing function compositions with high precedence. If `f` and `g` are functions, then `f . g` is the function which first does `g` and then `f`. Calling `(f . g) x` is equivalent to `f (g x)`; calling `(f . g . h) x` is equivalent to `f (g (h x))`. By chaining the functions, it is possible to define a function without referencing its arguments ("point-free" style).
```
> :t (.)
(.) :: (b -> c) -> (a -> b) -> a -> c
...
The backslash operator ("\") creates an unnamed lambda function. The variables introduced inside a lambda expression are local to that expression.
```
l1 x y = x * y
l2 x = \y -> x * y
l3 = \x -> \y -> x * y
...
A section is syntactic sugar for a lambda operation. For example, the section `(+2)` is equivalent to `\x -> x + 2`.
```
> map (4+) [1,2,3]
[5,6,7]
...
Record syntax allows you to name the values in a datatype. Records have the syntax: `{ name :: value, name2 :: value2 }`. Haskell automatically creates a function that allows you retrieve the value inside a structure. Naming the records also allows you to specify the fields in any order.
```
...
> :t year
year :: Date -> Int
> year (Date 1900 05 20)
1900
> month (Date 1900 05 20)
5
> let birthday = Date { month = 03, day = 14, year = 2005 }
> :t birthday
birthday :: Date
> year birthday
2005
...
The `Monoid` type class defines types that have a single associative binary operation (`mappend` or the infix operator `<>`) and an identity element (`mempty`). Instances of `Monoid` class should obey the following laws: 1) `mappend mempty x = x`; 2) `mappend x empty = x`; 3) `mappend x (mappend y z) = mappend (mapped (x y) z`; 4) `mconcat = foldr mappend mempty`.
```
> :t mempty
mempty :: Monoid a => a
> :t mappend
...
The list `Monoid` instance, consists of the empty lists for identity element, and the list concatenation operator `++` for the associvative operation. The `Sum` monoid has `0` as the identity element, and `+` for the associative operation. The Product `Monoid` instance has `1` as the identity element, and `*` for the associative operation.
```
...
> :m Data.Monoid
Data.Monoid> mappend (Product 3) (Product 5)
Product {getProduct = 15}
Data.Monoid> (Product 10) <> mempty
Product {getProduct = 10}
Data.Monoid> mappend (Sum 3) mempty
Sum {getSum = 3}
```
...
The `Functor` class creates a mapping between two different categories, while preserving their structure. An instance of `Functor` must accept a single type argument.
```
class Functor f  where
    fmap :: (a -> b) -> f a -> f b
...
An `Applicative` is a monoidal functor. The `pure` function wraps a value inside an applicative structure. The `(<*>)` function performs sequential application, similar to the "do" expression.
```
...
The list `Applicative` maps a list of functions over a list of values. The `Applicative Maybe` applies a partial function if both the first and the second arguments are `Just`, else it returns `Nothing`.
```
> [(*3), (+5)] <*> [1, 2, 3]
[3,6,9,6,7,8]
...
`The `lift` family of functions in `Control.Applicative` are like the `fmap` function but with an `Applicative` typeclass constraint.
```
...
Control.Applicative> liftA (+5) [1, 2, 3]
[6,7,8]
...
Haskell uses the `Monad` type class to implement monads. The monad of type `M a` is interpreted as a statement in an imperative language `M` that returns a value of type `a`. Every monad is also a `Functor` and an `Applicative`, which means that the `fmap`, `pure`, and `(<*>)` functions can be used on them.
...
`IO` actions are said to be "impure", because they introduce side-effects. The `IO` monad allows Haskell to represent actions as values. Functions that have the `IO ()` type do not pass on values to later functions. The `main :: IO ()` function is the starting point of a Haskell program, and can be composed of many smaller IO computations.

A value of type `IO a` consists of some number of I/O operations that will eventually produce a value of type `a`. For example, a value of type `IO String` is a description of a computation that will generate a String.

The IO monad is most commonly used with the "do" operator, which is essentially equivalent to the `(>>)` (then) operator. The `(>>)` then operator combines two or more IO computations. The `(>>=)` bind operator takes in a computation's intermediate value, performs a computation on the value, which can then passed to further computations.
```
(>>) :: IO a -> IO b -> IO b
(>>=) :: IO a -> (a -> IO b) -> IO b
...
Modules are the main way Haskell code is organized. Module names must begin with a capital letter. A module source code file should start with the line: `module ModuleName where`.
...
The "qualified" keyword is useful when you are dealing with conflicting namespaces across libraries. It forces you to specify the entire function name path. The "as" keyword allows you to rename an imported module name.
```
...
The `Test.QuickCheck` package provides testing functions. To use this package, rewrite your functions to return a `Bool` value. The `quickCheck` function then generates automated input values.
```
> :m Test.QuickCheck
...
The `Foreign` library has modules that support functions imported from other languages. For example the `Foreign.C` package has C functionality. Imported routines must be marked as either "safe" or "unsafe".




References: [^allen] [^esposito] [^lipovaca] [^upenn] [^wikibooks_haskell] [^wikibooks_scm]

[^allen]: Allen, Christopher (2015). Haskell Programming From First Principles.
