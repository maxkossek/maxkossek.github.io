...
The `:kind` or `:k` interactive command evalautes the kind of a data type. A data type of kind `* -> *` maps one kind `*` to another kind `*`.
```
> :k Int
Int :: *
...
The `Monoid` type class defines types that fulfill the identity and associativity property. Lists and the addition and multiplication operations are basic examples of monoids.
```
x <> mempty = x
mempty <> x = x
(x <> y) <> z = x <> (y <> z)
