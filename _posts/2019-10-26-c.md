...
Include the library <stdio.h> for standard input and output functions. The printf() function is used to print to STDOUT. The % character signals a formatting directive.
```
printf("Float: %3.2f", x);
```
...
Error messages can be written to STDERR using fprintf(). The strerror() function in <errno.h> prints a textual description for an error code.
```
fprintf(stderr, "Can't open %s\n", *argv);
fprintf(stderr, "EFAULT: %s\n", strerror(EFAULT));
```

fopen() is used for file input and output. Modes: "r" = read, "w" = write, "a" = append. Use fgets() to or getc() to read contents from a file. putc() writes a single character to a file.
```
...
Local variables must be declared with a data type. Global variables are declared with the "extern" keyword. The "#define" keyword is used to define a macro. A macro replaces all occurrences of a string with the specified value at compile time. The "const" keyword makes a variable immutable.
```
int x, y;
float x = 3.534;
...
Function prototypes have to be declared before the main() function or in a ".h" file. Specifying the parameters names is optional, but recommended.
```
int func_name(int x1, int x2);
```
...
Enumerations map integers to names in the program. By default the first element in the enum gets the value 0, the second 1, and so on. Most conventions use uppercase names for enumeration identifiers.
```
enum boolean {NO, YES};
