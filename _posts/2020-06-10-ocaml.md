...
Expressions in the toplevel must be separated using the double semicolon (";;"). The double semicolon is unneccessary in source code files. The single semicolon chains together a number of expressions that have the return value `()`.
```
# let x = 5 ;;
val x : int = 5
# print_string "Hello" ;
...
`ref`
: Reference type (pointer to a mutable value).
...
Relational operators: `=` (structural equality), `<>` (structural inequality), `==` (physical equality), `!=` (physical inequality), `<`, `<=`, `>`, `>=`.
```
# "abc" = "abc" ;;
- : bool = true
# "abc" == "abc" ;;
...
The "let" keyword can be used to create a function defintion. The OCaml compiler can perform type inference on most operations. A function application has the syntax: `e0 e1 e2 ... en`. Function parameters can be passed with or without parenthesis ("()"). The infix operator `|>` allows you to write a function application from left to right. Function applications are left associative, while function types are right associative.
```
# let inc x = x + 1 ;;
val inc : int -> int = <fun>
# inc(1) ;;
...
The fields of a record can be made mutable by using the "mutable" keyword.
```
...
The `ref` type stores a single mutable value. It is defined as a record with one field called `contents`.
```
# let p = { contents = 0 } ;;
...
The `ref` type also has a number of syntactic shortcuts. For example, to create a `ref` you can write: `let r = ref e`. For assignment use the assignment operator ":=", so: `r := e`. To dereference the contents of the `ref` use the syntax: `!r`.
```
# let q = ref 5 ;;
...
Arrays are a multiple data structure that store a list of elements of the same type. Arrays are more memory efficient than lists.
...
Compiling module files creates a ".cmo" compiled module object file.

The `#load` toplevel command loads a compiled module object file and makes it avilable in the interpreter session. The `#use` command directly loads the textual contents of a file into the session.
...
Functionality for dealing with floating point values.
...
`Int64` values: `Int64.zero`, `Int64.one`, `Int64.minus_one`, `Int64.max_int`, `Int64.min_int`.

Arithemtic functions: `Int64.neg`, `Int64.add`, `Int64.sub`, `Int64.mul`, `Int64.div`, `Int64.unsigned_div`, `int64.rem`, `Int64.abs`.

Bitwise functions: `Int64.logand`, `Int64.logor`, `Int64.logand`, `Int64.logxor`, `Int64.lognot`, `Int64.shift_left`, `Int64.shift_right`, `Int64.shift_right_logical`.

Conversion functions: `Int64.of_int`, `Int64.to_int`, `Int64.unsigned_to_int`, `Int64.of_float`, `Int64.to_float`, `Int64.of_int32`, `Int64.to_int32`, `Int64.of_string`, `Int64.of_string_opt`, `Int64.to_string`,

Comparison functions: `Int64.compare`, `Int64.unsigned_compare`, `Int64.equal`.
...
List properties: `List.length`, `List.compare_lengths`, `List.compare_length_with`.

List construction: `List.cons`, `List.append`, `List.rev_append`, `List.concat`.

List access: `List.hd`, `List.tl`, `List.nth`.

List modification: `List.rev`, `List.flatten`.

List iterators: `List.iter`, `List.iteri`, `List.map`, `List.mapi`, `List.rev_map`, `List.filter_map`, `List.concat_map`, `List.fold_left`, `List.fold_right`.

List iterators on two lists: `List.iter2`, `List.map2`, `List.rev_map2`, `List.fold_left2`, `List.fold_right2`.

List scanning: `List.for_all`, `List.exists`, `List.for_all2`, `List.exists2`, `List.mem`, `List.memq`.

List searching: `List.find`, `List.find_opt`, `List.find_map`, `List.filter`, `List.find_all`, `List.partition`.

List sorting: `List.sort`, `List.stable_sort`, `List.fast_sort`, `List.sort_uniq`, `List.merge`.

Association lists: `List.assoc`, `List.assoc_opt`, `List.assq`, `List.assq_opt`, `List.mem_assoc`, `List.assq`, `List.remove_assoc`, `List.remove_assq`.

Lists of pairs: `List.split`, `List.combine`.



### `Stdlib` module

All the functions in `Stdlib` can be called without having to prefix `Stdlib`.

Exceptions: `raise`, `raise_notrace`, `invalid_arg`, `failwith`.

Exception types: `Exit`, `Match_failure`, `Assert_failure`, `Invalid_argument`, `Failure`, `Not_found`, `Out_of_memory`, `Stack_overflow`, `Sys_error`, `End_of_file`, `Division_by_zero`, `Sys_blocked_io`, `Undefined_recursive_module`.

Geometric functions: `cos`, `sin`, `tan`, `acos`, `asin`, `atan`, `atan2`, `hypot`, `cosh`, `sinh`, `tanh`.

Math functions: `sqrt`, `exp`, `log`, `log10`, `ceil`, `floor`, `mod_float`.

Output functions: `print_char`, `print_string`, `print_bytes`, `print_int`, `print_float`, `print_endline`, `print_newline`.

Unit operations: `ignore`.
