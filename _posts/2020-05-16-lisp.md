...
FORMAT takes in a number of formatting directives, that function similar to directives of printf in the C language.
...
QUOTE signifies Lisp to not evaluate the following expression.
```
> '(quote (+ 1 2))
'(+ 1 2)
> '(+ 1 2)
...
By convention, global variables names are written between "*" characters (e.g. *GLOBAL-VAR*) and constant variable names are placed between "+" characters (e.g. +CONST-VAR+).
